-- 补齐 Hibernate 时代可能缺失的表（IF NOT EXISTS，可重复执行）
-- 配合 baseline-on-migrate：旧库跳过 V1，由本脚本补 currency / md_* 等
...
-- ========== 主数据 ==========
CREATE TABLE IF NOT EXISTS md_us_state (
    id       BIGSERIAL PRIMARY KEY,
    code     VARCHAR(8)  NOT NULL UNIQUE,
    name_zh  VARCHAR(64) NOT NULL
);

CREATE TABLE IF NOT EXISTS md_dest_city (
    id        BIGSERIAL PRIMARY KEY,
    state_id  BIGINT       NOT NULL REFERENCES md_us_state(id),
    name      VARCHAR(128) NOT NULL
);

CREATE TABLE IF NOT EXISTS md_dest_zip (
    id        BIGSERIAL PRIMARY KEY,
    city_id   BIGINT      NOT NULL REFERENCES md_dest_city(id),
    zip_code  VARCHAR(32) NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_md_dest_city_state ON md_dest_city(state_id);
CREATE INDEX IF NOT EXISTS idx_md_dest_zip_city ON md_dest_zip(city_id);
CREATE INDEX IF NOT EXISTS idx_md_dest_zip_code ON md_dest_zip(zip_code);

CREATE TABLE IF NOT EXISTS md_global_port (
    id              BIGSERIAL PRIMARY KEY,
    code            VARCHAR(8)   NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS md_inland_por (
    id      BIGSERIAL PRIMARY KEY,
    name    VARCHAR(128) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_md_inland_por_pol ON md_inland_por(pol_id);

CREATE TABLE IF NOT EXISTS md_data_sync_meta (
    id              BIGSERIAL PRIMARY KEY,
...
-- ========== 成本库 ==========
CREATE TABLE IF NOT EXISTS cost_road (
    id                      BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS cost_sea (
    id                    BIGSERIAL PRIMARY KEY,
    origin                VARCHAR(128),
...
CREATE TABLE IF NOT EXISTS cost_fumigation (
    id                   BIGSERIAL PRIMARY KEY,
    port                 VARCHAR(64),
    station              VARCHAR(64),
...
CREATE TABLE IF NOT EXISTS cost_table_template (
    id          BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS quote_order_line (
    id           BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_quote_order_line_quote ON quote_order_line(quote_id);
